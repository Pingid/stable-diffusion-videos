...
def upload_folder_chunked(
    ...
    url = create_repo(repo_id, exist_ok=True,
                      private=private, repo_type='dataset')
    ...
    for i, file_paths in enumerate(get_groups_of_n(n, get_all_files(root))):
        ...
        operations = [
            CommitOperationAdd(
                path_in_repo=f"{file_path.parent.name}/{file_path.name}", path_or_fileobj=str(file_path))
...
def generate_input_batches(pipeline, prompts, seeds, batch_size, height, width):
    ...
    for i, (prompt, seed) in enumerate(zip(prompts, seeds)):
        ...
        embeds_batch = (
            embeds if embeds_batch is None else torch.cat(
                [embeds_batch, embeds])
        )
...
def generate_images(
    ...
    if len(seeds) != num_images:
        raise ValueError(
            "Number of seeds must be equal to batch_size * num_batches.")

    if upsample:
        if getattr(pipeline, 'upsampler', None) is None:
            pipeline.upsampler = RealESRGANModel.from_pretrained(
                'nateraw/real-esrgan')
    ...
    cfg = dict(
        ...
        device_name=torch.cuda.get_device_name(
            0) if torch.cuda.is_available() else 'unknown',
    )
    ...
    for batch_idx, embeds, noise in generate_input_batches(pipeline, [prompt] * num_images, seeds, batch_size, height, width):
        ...
        for image in images:
            frame_filepath = save_path / \
                f"{seeds[frame_index]}{image_file_ext}"
    ...
    if push_to_hub:
        upload_folder_chunked(repo_id, save_path,
